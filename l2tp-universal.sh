...
# 5) Выводит цветную таблицу пользователей: username, password, internal IP, outgoing IP
...
create_users() {
  ...
  for i in $(seq 1 "$count"); do
    ...
    echo -e "${user}\t${pass}" >> /etc/ppp/l2tp-secrets.txt
  done
...
collect_client_info() {
  # Распределение "исходящего IP" клиентам — логически одинаковый для всех, но при множественных SNAT правилах возможна балансировка.
  # Здесь показываем все доступные внешние IP сервера.
  mapfile -t ips < <(get_all_public_ips)
  ...
  # Цветная таблица
  printf "\n${CYAN}%-18s %-18s %-16s %-16s${NC}\n" "USERNAME" "PASSWORD" "INTERNAL_IP" "OUTGOING_IP" | tee -a "$OUT_FILE"
  # Внутренний IP назначается динамически pppd при подключении — заранее неизвестен.
  # Мы укажем placeholder "assigned-on-connect".

  while IFS=$'\t' read -r u p; do
    [[ -z "$u" || -z "$p" ]] && continue
    printf "${GREEN}%-18s${NC} %-18s %-16s %-16s\n" "$u" "$p" "assigned-on-connect" "${ips[0]:-N/A}" | tee -a "$OUT_FILE"
